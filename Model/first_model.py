...
# Define a function to build the CNN with Attention model
def build_cnn_attention_model():
    ...
    attention = Attention()([conv_layer, conv_layer])
    pool_layer = GlobalMaxPooling1D()(attention)
    output_layer = Dense(n, activation='softmax')(pool_layer)
    model = Model(inputs=input_layer, outputs=output_layer)
    return model

# Create the CNN with Attention model
cnn_attention_model = build_cnn_attention_model()

# Compile the model
cnn_attention_model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])

# Print model summary
cnn_attention_model.summary(line_length=120)
...
for epoch in range(number_of_epoch):
    ...
    cnn_attention_model.fit(train_input_list, train_y, verbose=1, epochs=1, batch_size=32)
    
    train_y_pred = predict_classes(cnn_attention_model.predict(train_input_list_copy, verbose=1))
    val_y_pred = predict_classes(cnn_attention_model.predict(val_input_list, verbose=1))
    test_y_pred = predict_classes(cnn_attention_model.predict(test_input_list, verbose=1))
    ...
    if val_max_f1 == val_all[3]: 
        ...
        if save_model : 
            cnn_attention_model.save('./Model/cnn_attention_relation_extraction_model.keras')
            print("Model saved", "cnn_attention_relation_extraction_model.keras")
        step_epoch = 0
